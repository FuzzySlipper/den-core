...
| `den-services/gateway` | Current gateway/proxy implementation for delivery/wake routing, adapter bindings, claim/attempt state, echo suppression, outage pause/resume, and local sentinel coordination. | The work changes how events become deliveries, how agents are woken, routing/claim/retry/dedupe/terminalization policy, or gateway diagnostics. File implementation tasks in `den-services`; edit the `gateway` module/path. | Channel data ownership, Core canonical state, Hermes implementation details beyond adapter contracts, or the retired `den-gateway` archive. |
| `den-gateway` | **Retired / historical-only archive.** | Only deliberate archive/decommission/history updates. | Current gateway/proxy/delivery/wake implementation; use `den-services/gateway`. |

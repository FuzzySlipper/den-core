...
Task #3324 replaces SQLite FTS5 document and knowledge searches with Postgres
full-text search while keeping SQLite behavior intact for the current provider.
...
Snippets now come from `ts_headline` on the matched body text. SQLite FTS5 keeps
`snippet(...)` and its virtual-table `rank` ordering. Postgres ranks with

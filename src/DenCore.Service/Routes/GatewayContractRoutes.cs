...
public static class GatewayContractRoutes
{
    public static void MapGatewayContractRoutes(this WebApplication app)
    {
        ...
        group.MapGet("/readiness", async (HttpContext http, DenCoreOptions options, DbConnectionFactory db) =>
        {
    ...
    private static bool HasServiceToken(DenCoreOptions options) => !string.IsNullOrWhiteSpace(options.GatewayContract.ServiceToken);

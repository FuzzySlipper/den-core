...
public sealed class PostgresDatabaseInitializer : IDatabaseInitializer
{
    ...
    internal const string InitialSchema = """
        ...
        CREATE TABLE IF NOT EXISTS agent_runs (
            ...
            state                           TEXT NOT NULL DEFAULT 'unknown'
                                            CHECK (state IN (
                                                'running', 'retrying', 'aborting',
                                                'rerun_requested', 'rerun_accepted',
        ...
        CREATE TABLE IF NOT EXISTS agent_workspaces (
            ...
            state                 TEXT NOT NULL DEFAULT 'active'
                                  CHECK (state IN ('planned', 'active', 'review', 'complete', 'failed', 'archived')),
            created_by_run_id     TEXT REFERENCES agent_runs(run_id) ON DELETE SET NULL,
            dev_server_url        TEXT,
            preview_url           TEXT,
            cleanup_policy        TEXT NOT NULL DEFAULT 'keep'
                                  CHECK (cleanup_policy IN ('keep', 'delete_worktree', 'archive')),
        ...
        CREATE TABLE IF NOT EXISTS desktop_git_snapshots (
            ...
            state                 TEXT NOT NULL DEFAULT 'ok'
                                  CHECK (state IN ('ok', 'path_not_visible', 'not_git_repository', 'git_error', 'source_offline', 'missing')),
            branch                TEXT,
        ...
        CREATE TABLE IF NOT EXISTS desktop_session_events (
            ...
            event_type         TEXT NOT NULL
                               CHECK (event_type IN (
                                   'created', 'discovered', 'status_changed', 'capabilities_changed',
                                   'attached', 'detached', 'input_sent', 'resize_requested',
                                   'terminate_requested', 'terminate_completed', 'reconnect',
                                   'reconnect_requested', 'reconnected', 'lease_acquired',
                                   'lease_lost', 'lease_conflict', 'warning', 'crashed',
                                   'exited', 'snapshot_published', 'snapshot_publish_failed'
                               )),
            ...
            created_at         TEXT NOT NULL DEFAULT (
                ...
                || 'T' ||
                to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'HH24:MI:SS')
                || 'Z')
        );
        ...
        CREATE TABLE IF NOT EXISTS collaboration_segments (
            ...
            segment_type    TEXT NOT NULL CHECK (segment_type IN ('heading', 'paragraph', 'code_block', 'list', 'block_quote')),
            raw_markdown    TEXT NOT NULL,
            text            TEXT,
        ...
        CREATE INDEX IF NOT EXISTS idx_collaboration_sessions_project_updated
            ON collaboration_sessions(project_id, updated_at DESC, id DESC);
        ...
        CREATE INDEX IF NOT EXISTS idx_collaboration_turns_session_order
            ON collaboration_turns(session_id, turn_order);
        CREATE INDEX IF NOT EXISTS idx_collaboration_segments_turn_order
            ON collaboration_segments(turn_id, sequence_number);
        CREATE INDEX IF NOT EXISTS idx_collaboration_annotations_session_updated
            ON collaboration_annotations(session_id, updated_at DESC, id DESC);
        CREATE INDEX IF NOT EXISTS idx_collaboration_annotations_segment
            ON collaboration_annotations(segment_id, updated_at DESC, id DESC);
        CREATE INDEX IF NOT EXISTS idx_collaboration_drafts_session_updated
            ON collaboration_response_drafts(session_id, updated_at DESC, id DESC);
        """;
    ...
    internal const string LateSurfaceTablesSchema = """
        ALTER TABLE IF EXISTS desktop_session_events
        ...
        CREATE TABLE IF NOT EXISTS blackboard_entries (
            ...
            created_at           TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS')),
            updated_at           TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS')),
            last_accessed_at     TEXT NOT NULL DEFAULT (to_char(CURRENT_TIMESTAMP AT TIME ZONE 'UTC', 'YYYY-MM-DD HH24:MI:SS'))
        );
        ...
        ------------------------------------------------------------
        -- CONSOLIDATION TOPICS / CLIP CURATION
        ------------------------------------------------------------
        ...
        ------------------------------------------------------------
        -- CAPABILITY REGISTRY / INVOCATION AUDIT
        ------------------------------------------------------------

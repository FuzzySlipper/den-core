...
public static partial class MemoryRoutes
{
    ...
    public static void MapMemoryRoutes(this WebApplication app)
    {
        ...
        group.MapGet("/spaces", (HttpContext http, DenMcpOptions options) =>
            IsAuthorized(http, options)
                ? Results.Ok(new MemorySpacesResponse(InitialSpaces))
                : Results.Unauthorized());

        group.MapPost("/entries", async (HttpContext http, DenMcpOptions options, IDocumentRepository repo, string projectId, StoreMemoryEntryRequest req) =>
        {
        ...
        group.MapGet("/entries/{entryId}", async (HttpContext http, DenMcpOptions options, IDocumentRepository repo, string projectId, string entryId) =>
        {
            if (!IsAuthorized(http, options))
                return Results.Unauthorized();
        ...
        group.MapPost("/search", async (HttpContext http, DenMcpOptions options, IDocumentRepository repo, string projectId, SearchMemoryEntriesRequest req) =>
        {
            if (!IsAuthorized(http, options))
                return Results.Unauthorized();
        ...
        group.MapDelete("/entries/{entryId}", async (HttpContext http, DenMcpOptions options, IDocumentRepository repo, string projectId, string entryId) =>
        {
            if (!IsAuthorized(http, options))
                return Results.Unauthorized();

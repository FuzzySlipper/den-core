...
public class WorkerPoolRepositoryTests : IAsyncLifetime
{
    ...
    /// <summary>
    /// Seed a pi_session in 'launching' state directly into the database.
    /// WorkerPoolRepository.DetectOrphanedWorkerRunsAsync cross-references
    /// pi_sessions with worker_assignments to find orphans.
    /// </summary>
    private async Task SeedLaunchingSessionAsync(
        string sessionId,
        string projectId,
        string runId,
        DateTime? createdAt = null)
    {
        ...
        cmd.CommandText = """
            INSERT INTO pi_sessions (
                session_id, project_id, task_id, run_id, title, tool_profile,
                host_id, tmux_session_name, state, state_reason,
                launch_profile_kind, launch_profile_id,
                launch_profile_json, launch_command_json, launch_command_display,
                created_at, updated_at
            ) VALUES (
                @sessionId, @projectId, @taskId, @runId, 'Test Worker', 'coder',
                'test-host', @tmuxSession, 'launching', 'registered spawned-Hermes worker run; waiting for bridge launch',
                'spawned_hermes', @sessionId,
                '{}', '[]', 'test',
                @createdAt, @createdAt
            )
            """;
        ...
        cmd.Parameters.AddWithValue("@tmuxSession", $"spawned-hermes-{sessionId}");
        cmd.Parameters.AddWithValue("@createdAt", (createdAt ?? DateTime.UtcNow).ToString("o"));
        await cmd.ExecuteNonQueryAsync();
    }

    [Fact]
    public async Task OrphanDetect_FindsStaleLaunchingSession()
    {
        // Create a stale launching session (created 20 minutes ago)
        var staleTime = DateTime.UtcNow.AddMinutes(-20);
        await SeedLaunchingSessionAsync("orphan-session-1", "test-proj", "run-orphan-1", createdAt: staleTime);

        var orphans = await _repo.DetectOrphanedWorkerRunsAsync(staleThresholdMinutes: 10);
        Assert.Single(orphans);
        Assert.Equal("orphan-session-1", orphans[0].SessionId);
        Assert.Equal("run-orphan-1", orphans[0].RunId);
        Assert.Equal("test-proj", orphans[0].ProjectId);
        Assert.True(orphans[0].IsStale);
        Assert.True(orphans[0].NoActiveAssignment);
        Assert.False(orphans[0].HasBusyPoolMember);
    }

    [Fact]
    public async Task OrphanDetect_IgnoresFreshLaunchingSession()
    {
        // Create a fresh launching session (created 1 minute ago)
        await SeedLaunchingSessionAsync("fresh-session-1", "test-proj", "run-fresh-1", createdAt: DateTime.UtcNow.AddMinutes(-1));
    ...
    [Fact]
    public async Task OrphanDetect_IgnoresNonLaunchingSession()
    {
        // Create a running session (not stuck in launching)
        await SeedLaunchingSessionAsync("running-session-1", "test-proj", "run-running-1", createdAt: DateTime.UtcNow.AddMinutes(-20));

        // Manually update it to running
        await using (var conn = await _testDb.Db.CreateConnectionAsync())
        await using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = "UPDATE pi_sessions SET state = 'running' WHERE session_id = 'running-session-1'";
            await cmd.ExecuteNonQueryAsync();
        }
    ...
    [Fact]
    public async Task OrphanDetect_SeesActiveAssignmentAsNotOrphaned()
    {
        // Create a pool member and lease it
        await SeedMemberAsync("orphan-worker-1", profileIdentity: "spawned-coder");
        var lease = await _repo.LeaseAvailableWorkerAsync(new LeaseWorkerInput
        {
            ProjectId = "test-proj",
            Role = "coder",
            AssignedBy = "runner",
            RunId = "run-has-assignment",
            PreferredWorkerIdentity = "orphan-worker-1",
        });
        Assert.NotNull(lease);

        // Create a launching session with the same run_id that has an active assignment
        await SeedLaunchingSessionAsync("session-with-assignment", "test-proj", "run-has-assignment", createdAt: DateTime.UtcNow.AddMinutes(-20));

        var orphans = await _repo.DetectOrphanedWorkerRunsAsync(staleThresholdMinutes: 10);
        // The session should still be detected because it's in launching state,
        // but NoActiveAssignment should be false since it has an active assignment
        Assert.Single(orphans);
        Assert.False(orphans[0].NoActiveAssignment);
        Assert.True(orphans[0].HasBusyPoolMember);
        Assert.Equal("orphan-worker-1", orphans[0].BusyWorkerIdentity);
    }

    [Fact]
    public async Task ForceTerminate_UpdatesSessionToFailed()
    {
        var staleTime = DateTime.UtcNow.AddMinutes(-20);
        await SeedLaunchingSessionAsync("terminate-session-1", "test-proj", "run-terminate-1", createdAt: staleTime);

        var result = await _repo.ForceTerminateOrphanRunAsync("terminate-session-1", "test-runner", "stale orphan");

        Assert.True(result.WasTerminated);
        Assert.False(result.IsAlreadyTerminal);
        Assert.Equal("terminate-session-1", result.SessionId);
        Assert.Equal("run-terminate-1", result.RunId);

        // Verify session state changed to failed
        await using var conn = await _testDb.Db.CreateConnectionAsync();
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT state, state_reason FROM pi_sessions WHERE session_id = 'terminate-session-1'";
        await using var reader = await cmd.ExecuteReaderAsync();
        Assert.True(await reader.ReadAsync());
        Assert.Equal("failed", reader.GetString(0));
        Assert.Contains("stale orphan", reader.GetString(1));
    }

    [Fact]
    public async Task ForceTerminate_CreatesAuditEvent()
    {
        await SeedLaunchingSessionAsync("audit-session-1", "test-proj", "run-audit-1", createdAt: DateTime.UtcNow.AddMinutes(-20));

        await _repo.ForceTerminateOrphanRunAsync("audit-session-1", "test-runner");
    ...
    [Fact]
    public async Task ForceTerminate_ReleasesBusyPoolMember()
    {
        await SeedMemberAsync("release-worker-1", profileIdentity: "spawned-coder");
        var lease = await _repo.LeaseAvailableWorkerAsync(new LeaseWorkerInput
        {
            ProjectId = "test-proj",
            Role = "coder",
            AssignedBy = "runner",
            RunId = "run-release-1",
            PreferredWorkerIdentity = "release-worker-1",
        });
        Assert.NotNull(lease);

        await SeedLaunchingSessionAsync("release-session-1", "test-proj", "run-release-1", createdAt: DateTime.UtcNow.AddMinutes(-20));

        var result = await _repo.ForceTerminateOrphanRunAsync("release-session-1", "test-runner", "orphan cleanup");

        Assert.True(result.WasTerminated);
        Assert.True(result.ReleasedPoolMember);
        Assert.Equal("release-worker-1", result.ReleasedWorkerIdentity);
        Assert.True(result.ExpiredAssignment);
        Assert.Equal(lease.Id, result.ExpiredAssignmentId);

        // Verify member is back to available
        var member = await _repo.GetMemberAsync("release-worker-1");
        Assert.Equal(WorkerPoolStates.MemberAvailable, member!.Status);

        // Verify assignment is expired
        var assignment = await _repo.GetAssignmentAsync(lease.Id);
        Assert.Equal(WorkerPoolStates.Expired, assignment!.State);
    }

    [Fact]
    public async Task ForceTerminate_IdempotentOnAlreadyTerminalSession()
    {
        await SeedLaunchingSessionAsync("idempotent-session-1", "test-proj", "run-idempotent-1", createdAt: DateTime.UtcNow.AddMinutes(-20));

        // First termination
        var result1 = await _repo.ForceTerminateOrphanRunAsync("idempotent-session-1", "test-runner");
        Assert.True(result1.WasTerminated);

        // Second termination should be idempotent
        var result2 = await _repo.ForceTerminateOrphanRunAsync("idempotent-session-1", "test-runner");
        Assert.True(result2.IsAlreadyTerminal);
        Assert.False(result2.WasTerminated);
    }

    [Fact]
    public async Task ForceTerminate_ThrowsForMissingSession()
    {
        await Assert.ThrowsAsync<InvalidOperationException>(() =>
            _repo.ForceTerminateOrphanRunAsync("nonexistent-session", "test-runner"));
    }

    [Fact]
    public async Task Summary_ExcludesOrphanedLaunchingFromActive()
    {
        // Create a pool member and lease it
        await SeedMemberAsync("summary-worker", profileIdentity: "spawned-coder");
        var lease = await _repo.LeaseAvailableWorkerAsync(new LeaseWorkerInput
        {
            ProjectId = "test-proj",
            Role = "coder",
            AssignedBy = "runner",
            RunId = "run-summary-orphan",
            PreferredWorkerIdentity = "summary-worker",
        });
        Assert.NotNull(lease);

        // Create a stale launching session linked to the assignment
        await SeedLaunchingSessionAsync("summary-session", "test-proj", "run-summary-orphan", createdAt: DateTime.UtcNow.AddMinutes(-20));

        var summary = await _repo.GetSummaryAsync();

        // The orphaned assignment should NOT count as active
        Assert.Equal(0, summary.ActiveAssignments);
        Assert.Equal(1, summary.OrphanedLaunchingAssignments);
    }

    [Fact]
    public async Task Summary_NormalActiveAssignment_IncludedInActive()
    {
        await SeedMemberAsync("normal-worker", profileIdentity: "spawned-coder");
        var lease = await _repo.LeaseAvailableWorkerAsync(new LeaseWorkerInput
        {
            ProjectId = "test-proj",
            Role = "coder",
            AssignedBy = "runner",
            RunId = "run-normal-active",
            PreferredWorkerIdentity = "normal-worker",
        });
        Assert.NotNull(lease);

        // No launching session = no orphan => counts as active
        var summary = await _repo.GetSummaryAsync();
        Assert.Equal(1, summary.ActiveAssignments);
        Assert.Equal(0, summary.OrphanedLaunchingAssignments);
    }

    [Fact]
    public async Task Summary_RecentLaunchingAssignment_StillCountsActive()
    {
        await SeedMemberAsync("recent-worker", profileIdentity: "spawned-coder");
        var lease = await _repo.LeaseAvailableWorkerAsync(new LeaseWorkerInput
        {
            ProjectId = "test-proj",
            Role = "coder",
            AssignedBy = "runner",
            RunId = "run-recent-launch",
            PreferredWorkerIdentity = "recent-worker",
        });
        Assert.NotNull(lease);

        // Create a FRESH launching session (only 1 minute old) — not yet orphaned
        await SeedLaunchingSessionAsync("recent-session", "test-proj", "run-recent-launch", createdAt: DateTime.UtcNow.AddMinutes(-1));

        var summary = await _repo.GetSummaryAsync();
        // Still fresh, so should count as active
        Assert.Equal(1, summary.ActiveAssignments);
        Assert.Equal(0, summary.OrphanedLaunchingAssignments);
    }

    [Fact]
    public async Task OrphanDetect_MultipleOrphans()
    {
        await SeedLaunchingSessionAsync("orphan-a", "test-proj", "run-orphan-a", createdAt: DateTime.UtcNow.AddMinutes(-30));
        await SeedLaunchingSessionAsync("orphan-b", "test-proj", "run-orphan-b", createdAt: DateTime.UtcNow.AddMinutes(-15));
        // Fresh one should not be detected
        await SeedLaunchingSessionAsync("orphan-fresh", "test-proj", "run-orphan-fresh", createdAt: DateTime.UtcNow.AddMinutes(-2));

        var orphans = await _repo.DetectOrphanedWorkerRunsAsync(staleThresholdMinutes: 10);
        Assert.Equal(2, orphans.Count);
        Assert.Contains(orphans, o => o.SessionId == "orphan-a");
        Assert.Contains(orphans, o => o.SessionId == "orphan-b");
    }
}
